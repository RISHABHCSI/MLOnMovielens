...
def initialization(dataSet,k):
    ...
    # print cluster[1][0][:20]
    # print dataSet[10,:20]
def test():
    dataSet=loadData.loadTrainingData("u1.base")
    testData=loadData.loadTestData("u1.test")
    clusters,emptyPool,meanList=initialization(dataSet,10)
    # var=0
    while len(emptyPool):
        # print var
        ...
        # print randNes
        # print dataSet[user,:]
        # print meanList[randNes]
        # print len(meanList[randNes])
        # break
        ...
        if count:
            if count>=threshold:
                # for q in range(len(clusters[randNes])):
                for c in clusters[randNes]:
                    # print clusters[randNes][q][0]
                    if c[0]==minPerson:
